...
follow-up hardening work. Session issuance and invocation failures return a
stable `ErrorEnvelope` on the `error` field for policy denial, approval
required, active revocation, active kill switch, missing capability, missing
session, expired session, and invalid session-scope cases.
