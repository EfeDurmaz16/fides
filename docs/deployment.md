...
| Variable          | Default | Required | Description                                   |
| ----------------- | ------- | -------- | --------------------------------------------- |
| `SERVICE_API_KEY` | _(empty)_ | production | Shared API key for mutating endpoints. When unset, auth is disabled (development mode). |
...
2. Set `SERVICE_API_KEY` to a strong random value (generated via `openssl rand -hex 32`)
