...
and dry-run mode before policy preflight. The daemon then emits hash-only
...
request proves requester intent, and the signed result proves that the target
agent identity produced the invocation outcome; neither proof bypasses policy,
revocation, or evidence verification.
