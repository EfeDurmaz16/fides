...
describe('SessionGrant v2', () => {
  ...
  it('signs and verifies a session grant using the canonical signing model', async () => {
    ...
    expect(await verifySignedSessionGrantV2Issuer(signed)).toBe(true)
  })

  it('rejects session grant proofs whose verification method is not the issuer', async () => {
    const issuer = await createIdentityKeyPair()
    ...
    const signed = await signSessionGrantV2(grant, attacker.privateKey, attacker.did)

    expect(await verifySignedSessionGrantV2(signed)).toBe(true)
    expect(await verifySignedSessionGrantV2Issuer(signed)).toBe(false)
  })
